...
# Creating condtions variable 
```{r}
# List of condition variable names
Condition_vars <- c("Conditions_Cancer", "Conditions_Severe_Allergies", "Conditions_Seizures", "Conditions_Obesity",
                    "Conditions_Cardio_Disease", "Conditions_Rheumatological", "Conditions_Pulmonary", "Conditions_Diabetes",
                    "Conditions_Immuno", "Conditions_Pregnancy", "Conditions_Other", "Conditions_No_Conditions")

# Create a logical vector for missing values or "*"
missing_values <- rowSums(is.na(asu_merged[, Condition_vars]) | asu_merged[, Condition_vars] == "*") == 13

# Create a logical vector for values equal to "1" in the "no conditions" variable
no_conditions <- asu_merged$no_conditions == "1"

# Create a logical vector for at least one condition variable being true
at_least_one_condition <- rowSums(asu_merged[, Condition_vars] != "NA" & asu_merged[, Condition_vars] != "*") > 0

# Create the new variable 'conditions_new' based on the conditions
asu_merged$conditions_new <- NA
asu_merged$conditions_new[missing_values] <- "missing"
asu_merged$conditions_new[no_conditions] <- "no conditions"
asu_merged$conditions_new[at_least_one_condition] <- "at least 1"
```
...
# New conditions variable  
```{r}
Condition_vars = asu_merged$Condition_vars  %>% replace(is.na(.), "*")

Conditions_Condition_vars_pertable= PercTable(Condition_vars)
Conditions_Condition_vars_pertable
```
