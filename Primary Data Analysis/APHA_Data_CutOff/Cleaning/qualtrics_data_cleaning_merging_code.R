...
#Step 1: Preparing the data
  ##Loading the Qualtrics Data
    asu_qualtrics_data <- read_excel(here("Primary Data Analysis", "APHA_Data_Cutoff", "Raw_Data", "Health Literacy Survey (full survey)_May 17, 2023_09.11.xlsx"))
...
#Step 9: Loading the pen and paper survey
  #uploading the pen and paper excel file but changing the date variable to be saved as a date so that the values remain as they were in the MM/DD/YY format 
  
  asu_pen_paper_data <- read_excel("/Users/jacobmatta/Documents/R/ASU-Project-/Primary Data Analysis/APHA_Data_CutOff/Raw_Data/ASU_penandpaper_preandpost_merged_June9th.xlsx", 
  ...
  asu_pen_paper_data$ParticipantID <- toupper(asu_pen_paper_data$ParticipantID)
    ...
    #changing the date varibale to a text variable so it can be merged with the qualtrics survey 
     asu_pen_paper_data$Date <- as.character.Date(asu_pen_paper_data$Date)

#Step 10: Filtering both data sets to contain November - April 
  #cleaning qualtrics survey
  asu_qualtrics_Nov_thru_April <- asu_qualtrics_clean[!(grepl("^10|^12|^05|^06", asu_qualtrics_clean$Date)), ]
  #cleaning the pen and paper survey 
  asu_pen_paper_data_Nov_thru_April <- subset(asu_pen_paper_data, substr(Date, 6, 7) %in% c("11", "12", "01", "02", "03", "04"))
     
     


#Step 11: Merging the two data sets
  asu_merged <- rbind(asu_qualtrics_Nov_thru_April, asu_pen_paper_data_Nov_thru_April)



#Step 12: Exporting the file to excel
  output_file <- here("Primary Data Analysis", "APHA_Data_CutOff", "Clean_Data", "asu_merged.xlsx")
