...
# Creating the Risk variable where 1 = asu_merged and 2 = not asu_merged to COVID-19 and 3 = paricipants that left quesiton missing or answered do not know or don't know
...
#Seeing if it worked
table(asu_merged$N_Conditions_b)
...
```{r}
PercTable(N_Children_under5 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for number of children between 5-11

```{r}
PercTable(N_Children_5to11 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for number of children between 12-17

```{r}
PercTable(N_Children_12to17 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
```{r}
PercTable(Number_elderly_1b ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for the number of elderly family members aged 75+

```{r}
PercTable(Number_elderly_2b ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
```{r}
PercTable(N_Conditions_b ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
