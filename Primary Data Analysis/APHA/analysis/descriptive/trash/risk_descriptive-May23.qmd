...
# This file can be deleted. AM not using CopyOfrisk_descriptive-May17.qmd but have not deleted this file just incase the code it needed
...
# Creating the Exposure variable where 1 = Exposed and 2 = not Exposed to COVID-19 and 3 = participants that left question missing or answered do not know or don't know
...
# Creating the Risk variable where 1 = Not at risk and 2 = at Risk to COVID-19 and 3 = participants that reported to being neutral
...
asu_merged$Risk[!(asu_merged$Question_1 %in% c(1:5))] <- 4
...
```{r}
asu_merged$Risk <- ifelse(asu_merged$Risk == 1, "Not at risk", 
                       ifelse(asu_merged$Risk == 2, "Neutral",
                              ifelse(asu_merged$Risk == 3, "At risk",
                                     ifelse(asu_merged$Risk == 4, "Missing", NA))))

asu_merged$Zip[!(asu_merged$Zip %in% c(31701, 31705, 31707))] <- "Other" 

PercTable(Zip ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Gender ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
asu_merged$Age <- ifelse(asu_merged$Age == 1, "18-24 years old", 
                                 ifelse(asu_merged$Age == 2, "25-34 years old",
                                        ifelse(asu_merged$Age == 3, "35-46 years old",
...
PercTable(Age ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Race_Ethnicity ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Education ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Language ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Income ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Family_household ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Children ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for number of children below 5 years old

```{r}
PercTable(Number_children_1 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for number of children between 5-11

```{r}
PercTable(Number_children_2 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for number of children between 12-17

```{r}
PercTable(Number_children_3 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for elderly family members

```{r}
PercTable(Elderly_family ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for the number of elderly family members aged 65-74

```{r}
PercTable(Number_elderly_1 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for the number of elderly family members aged 75+

```{r}
PercTable(Number_elderly_2 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(`Health Insurance` ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Cancer ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Severe_Allergies ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Seizures ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Obesity ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Cardio_Disease ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Rheumatological ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Pulmonary ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Diabetes ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Immuno ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Pregnancy ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_Other ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Conditions_No_Conditions ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
```{r}
##Question_17
asu_merged$Question_17 <- ifelse(asu_merged$Question_17 == 1, "Strongly Disagree", 
                            ifelse(asu_merged$Question_17 == 2, "Disagree",
                                   ifelse(asu_merged$Question_17 == 3, "Neutral",
                                          ifelse(asu_merged$Question_17 == 4, "Agree",
                                                 ifelse(asu_merged$Question_17 == 5, "Strongly Agree", NA)))))

PercTable(Question_17 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
```{r}
##Question_18
asu_merged$Question_18 <- ifelse(asu_merged$Question_18 == 1, "Strongly Disagree", 
                            ifelse(asu_merged$Question_18 == 2, "Disagree",
                                   ifelse(asu_merged$Question_18 == 3, "Neutral",
                                          ifelse(asu_merged$Question_18 == 4, "Agree",
                                                 ifelse(asu_merged$Question_18 == 5, "Strongly Agree", NA)))))

PercTable(Question_18 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_19 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_20_Basic_Health ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_20_COVID_19_Vacc ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_20_Improving_ventilation ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_20_Getting_tested ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_20_Following_recommendations ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_20_Staying_home ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_20_Seeking ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_20_Avoiding ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_21 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_22 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_23 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_24 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_25 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_26 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_27 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_28 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_29 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_30_Physician ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_30_Nurse_Practitioner ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_30_Registered_Nurse ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_30_Community_Health_Advocate ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_30_Community_Member ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_30_Other ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for Question_31: Who provided the information during the event today? specify

```{r}
PercTable(Question_31 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_32 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_33 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_34 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_35 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_36 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for Question_37: Please provide the name of the person (s) or organization (s) who provided the information

```{r}
PercTable(Question_37 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_38_Handout_flier ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_38_Social_media ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_38_Standard_media ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_38_Community_Health_Advocate ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_38_Church_service ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_38_Barbershop ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_38_Community_members ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_38_Others ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```

# Summary for Question_39: How did you hear about the activity you attended today? specify

```{r}
PercTable(Question_39 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_40 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_41 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_42 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_43 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_44 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_45 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_46 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_47 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_48 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_49 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_50 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_51 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_52 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_53 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_54 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_55 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_56 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_57 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_58 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_59 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_60 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_61 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_62 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_63 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_64 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_65 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_66 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_67 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_68 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_69 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_70 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_71 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_72 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_73 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_74 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_75 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_76 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_77 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(Question_78 ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
...
PercTable(data_source ~ Risk, data = asu_merged, rfrq = "111", margins = c(1,2))
```
