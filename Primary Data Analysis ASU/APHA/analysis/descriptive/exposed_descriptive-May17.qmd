...
using the asu_merged-May17 dataframe for this analysis because it holds all variables with numeric values but saved as character variables. This allows you to then convert the character variables to numeric variables, perform quality control, then filter the dataframe to only contain exposed participants. I can justify using this procedure because filtering the dataframe as numeric variables prevents additional missing being added to the dataset. 
```{r}
...
```{r}
#creating dataframe called "exposed" because we will only be working with participants that have been exposed to COVID-19
exposed = asu_merged

exposed = exposed %>% mutate_all(as.numeric)
...
```{r}
exposed$Gender <- ifelse(exposed$Gender == 1, "Male", 
                       ifelse(exposed$Gender == 2, "Female",
                              ifelse(exposed$Gender == 3, "I would rather not specify", NA)))

Gender_pertable= PercTable(exposed$Gender)
Gender_pertable

missing_values <- sum(is.na(exposed$Gender))
print(missing_values)
```

# Summary for age
```{r}
Age_pertable= PercTable(exposed$Age)
Age_pertable
```
...
race = exposed$Race_Ethnicity %>% replace(is.na(.), "*")
...
```{r}
Education = exposed$Education %>% replace(is.na(.), "*")
...
```{r}
Language = exposed$Language %>% replace(is.na(.), "*")
...
```{r}
Income = exposed$Income %>% replace(is.na(.), "*")
...
```{r}
Family_household = exposed$Family_household %>% replace(is.na(.), "*")
...
```{r}
Children = exposed$Children %>% replace(is.na(.), "*")
...
```{r}
Number_children_1 = exposed$Number_children_1 %>% replace(is.na(.), "*")
...
```{r}
Number_children_2 = exposed$Number_children_2 %>% replace(is.na(.), "*")
...
```{r}
Number_children_3 = exposed$Number_children_3 %>% replace(is.na(.), "*")
...
```{r}
Elderly_family = exposed$Elderly_family %>% replace(is.na(.), "*")
...
```{r}
Number_elderly_1 = exposed$Number_elderly_1 %>% replace(is.na(.), "*")
...
```{r}
Number_elderly_2 = exposed$Number_elderly_2 %>% replace(is.na(.), "*")
...
```{r}
#changes NA values to missing 
Health_Insurance = exposed$Health_Insurance %>% replace(is.na(.), "*")
...
```{r}
Conditions_Cancer = exposed$Conditions_Cancer %>% replace(is.na(.), "*")
...
```{r}
Conditions_Severe_Allergies = exposed$Conditions_Severe_Allergies %>% replace(is.na(.), "*")
...
```{r}
Conditions_Seizures = exposed$Conditions_Seizures %>% replace(is.na(.), "*")
...
```{r}
Conditions_Obesity = exposed$Conditions_Obesity %>% replace(is.na(.), "*")
...
```{r}
Conditions_Cardio_Disease = exposed$Conditions_Cardio_Disease %>% replace(is.na(.), "*")
...
```{r}
Conditions_Rheumatological = exposed$Conditions_Rheumatological %>% replace(is.na(.), "*")
...
```{r}
Conditions_Pulmonary = exposed$Conditions_Pulmonary %>% replace(is.na(.), "*")
...
```{r}
Conditions_Diabetes = exposed$Conditions_Diabetes %>% replace(is.na(.), "*")
...
```{r}
Conditions_Immuno = exposed$Conditions_Immuno %>% replace(is.na(.), "*")
...
```{r}
Conditions_Pregnancy = exposed$Conditions_Pregnancy %>% replace(is.na(.), "*")
...
```{r}
Conditions_Other = exposed$Conditions_Other %>% replace(is.na(.), "*")
...
```{r}
Conditions_No_Conditions = exposed$Conditions_No_Conditions %>% replace(is.na(.), "*")
...
```{r}
Question_1 = exposed$Question_1 %>% replace(is.na(.), "*")
...
```{r}
Question_2 = exposed$Question_2 %>% replace(is.na(.), "*")
...
```{r}
Question_3 = exposed$Question_3 %>% replace(is.na(.), "*")
...
```{r}
Question_4 = exposed$Question_4 %>% replace(is.na(.), "*")
...
```{r}
Question_5 = exposed$Question_5 %>% replace(is.na(.), "*")
...
```{r}
Question_6 = exposed$Question_6 %>% replace(is.na(.), "*")
...
```{r}
Question_7 = exposed$Question_7 %>% replace(is.na(.), "*")
...
```{r}
Question_8 = exposed$Question_8 %>% replace(is.na(.), "*")
...
```{r}
Question_9 = exposed$Question_9 %>% replace(is.na(.), "*")
...
```{r}
Question_10 = exposed$Question_10 %>% replace(is.na(.), "*")
...
```{r}
Question_11 = exposed$Question_11 %>% replace(is.na(.), "*")
...
```{r}
Question_12 = exposed$Question_12 %>% replace(is.na(.), "*")
...
```{r}
Question_13_Fever = exposed$Question_13_Fever %>% replace(is.na(.), "*")
...
```{r}
Question_13_Cough = exposed$Question_13_Cough %>% replace(is.na(.), "*")
...
```{r}
Question_13_Shortness_of_breath = exposed$Question_13_Shortness_of_breath %>% replace(is.na(.), "*")
...
```{r}
Question_13_Fatigue = exposed$Question_13_Fatigue %>% replace(is.na(.), "*")
...
```{r}
Question_13_Muscle_Body_Aches = exposed$Question_13_Muscle_Body_Aches %>% replace(is.na(.), "*")
...
```{r}
Question_13_Headache = exposed$Question_13_Headache %>% replace(is.na(.), "*")
...
```{r}
Question_13_Loss_of_Taste = exposed$Question_13_Loss_of_Taste %>% replace(is.na(.), "*")
...
```{r}
Question_13_Sore_Throat = exposed$Question_13_Sore_Throat %>% replace(is.na(.), "*")
...
```{r}
Question_13_Congestion = exposed$Question_13_Congestion %>% replace(is.na(.), "*")
...
```{r}
Question_13_Nausea = exposed$Question_13_Nausea %>% replace(is.na(.), "*")
...
```{r}
Question_13_Diarrhea = exposed$Question_13_Diarrhea %>% replace(is.na(.), "*")
...
```{r}
Question_13_Asymptomatic = exposed$Question_13_Asymptomatic %>% replace(is.na(.), "*")
...
```{r}
Question_13_Other = exposed$Question_13_Other %>% replace(is.na(.), "*")
...
```{r}
Question_14 = exposed$Question_14 %>% replace(is.na(.), "*")
...
```{r}
Question_15 = exposed$Question_15 %>% replace(is.na(.), "*")
...
```{r}
Question_16 = exposed$Question_16 %>% replace(is.na(.), "*")
...
```{r}
Question_17 = exposed$Question_17 %>% replace(is.na(.), "*")
...
```{r}
Question_18 = exposed$Question_18 %>% replace(is.na(.), "*")
...
```{r}
Question_19 = exposed$Question_19 %>% replace(is.na(.), "*")
...
```{r}
Question_20_Basic_Health = exposed$Question_20_Basic_Health %>% replace(is.na(.), "*")
...
```{r}
Question_20_COVID_19_Vacc = exposed$Question_20_COVID_19_Vacc %>% replace(is.na(.), "*")
...
```{r}
Question_20_Improving_ventilation = exposed$Question_20_Improving_ventilation %>% replace(is.na(.), "*")
...
```{r}
Question_20_Getting_tested = exposed$Question_20_Getting_tested %>% replace(is.na(.), "*")
...
```{r}
Question_20_Following_recommendations = exposed$Question_20_Following_recommendations %>% replace(is.na(.), "*")
...
```{r}
Question_20_Staying_home = exposed$Question_20_Staying_home %>% replace(is.na(.), "*")
...
```{r}
Question_20_Seeking = exposed$Question_20_Seeking %>% replace(is.na(.), "*")
...
```{r}
Question_20_Avoiding = exposed$Question_20_Avoiding %>% replace(is.na(.), "*")
...
```{r}
Question_21 = exposed$Question_21 %>% replace(is.na(.), "*")
...
```{r}
Question_22 = exposed$Question_22 %>% replace(is.na(.), "*")
...
```{r}
Question_23 = exposed$Question_23 %>% replace(is.na(.), "*")
...
```{r}
Question_24 = exposed$Question_24 %>% replace(is.na(.), "*")
...
```{r}
Question_25 = exposed$Question_25 %>% replace(is.na(.), "*")
...
```{r}
Question_26 = exposed$Question_26 %>% replace(is.na(.), "*")
...
```{r}
Question_27 = exposed$Question_27 %>% replace(is.na(.), "*")
...
```{r}
Question_28 = exposed$Question_28 %>% replace(is.na(.), "*")
...
```{r}
Question_29 = exposed$Question_29 %>% replace(is.na(.), "*")
...
```{r}
Question_30_Physician = exposed$Question_30_Physician %>% replace(is.na(.), "*")
...
```{r}
Question_30_Nurse_Practitioner = exposed$Question_30_Nurse_Practitioner %>% replace(is.na(.), "*")
...
```{r}
Question_30_Registered_Nurse = exposed$Question_30_Registered_Nurse %>% replace(is.na(.), "*")
...
```{r}
Question_30_Community_Health_Advocate = exposed$Question_30_Community_Health_Advocate %>% replace(is.na(.), "*")
...
```{r}
Question_30_Community_Member = exposed$Question_30_Community_Member %>% replace(is.na(.), "*")
...
```{r}
Question_30_Other = exposed$Question_30_Other %>% replace(is.na(.), "*")
...
```{r}
Question_31 = exposed$Question_31 %>% replace(is.na(.), "*")
...
```{r}
Question_32 = exposed$Question_32 %>% replace(is.na(.), "*")
...
```{r}
Question_33 = exposed$Question_33 %>% replace(is.na(.), "*")
...
```{r}
Question_34 = exposed$Question_34 %>% replace(is.na(.), "*")
...
```{r}
Question_35 = exposed$Question_35 %>% replace(is.na(.), "*")
...
```{r}
Question_36 = exposed$Question_36 %>% replace(is.na(.), "*")
...
```{r}
Question_37 = exposed$Question_37 %>% replace(is.na(.), "*")
...
```{r}
Question_38_Handout_flier = exposed$Question_38_Handout_flier %>% replace(is.na(.), "*")
...
```{r}
Question_38_Social_media = exposed$Question_38_Social_media %>% replace(is.na(.), "*")
...
```{r}
Question_38_Standard_media = exposed$Question_38_Standard_media %>% replace(is.na(.), "*")
...
```{r}
Question_38_Community_Health_Advocate = exposed$Question_38_Community_Health_Advocate %>% replace(is.na(.), "*")
...
```{r}
Question_38_Church_service = exposed$Question_38_Church_service %>% replace(is.na(.), "*")
...
```{r}
Question_38_Barbershop = exposed$Question_38_Barbershop %>% replace(is.na(.), "*")
...
```{r}
Question_38_Community_members = exposed$Question_38_Community_members %>% replace(is.na(.), "*")
...
```{r}
Question_38_Others = exposed$Question_38_Others %>% replace(is.na(.), "*")
...
```{r}
Question_39 = exposed$Question_39 %>% replace(is.na(.), "*")
...
```{r}
# Summary for Question_40:  Did your child or children also receive a COVID-19 vaccine today?  
Question_40 = exposed$Question_40 %>% replace(is.na(.), "*")
...
```{r}
Question_41 = exposed$Question_41 %>% replace(is.na(.), "*")
...
```{r}
Question_42 = exposed$Question_42 %>% replace(is.na(.), "*")
...
```{r}
Question_43 = exposed$Question_43 %>% replace(is.na(.), "*")
...
```{r}
Question_44 = exposed$Question_44 %>% replace(is.na(.), "*")
...
```{r}
Question_45 = exposed$Question_45 %>% replace(is.na(.), "*")
...
```{r}
Question_46 = exposed$Question_46 %>% replace(is.na(.), "*")
...
```{r}
Question_47 = exposed$Question_47 %>% replace(is.na(.), "*")
...
```{r}
Question_48 = exposed$Question_48 %>% replace(is.na(.), "*")
...
```{r}
# Summary for Question_49:   High blood pressure from the COVID-19 vaccine is of concern 
Question_49 = exposed$Question_49 %>% replace(is.na(.), "*")
...
```{r}
Question_50 = exposed$Question_50 %>% replace(is.na(.), "*")
...
```{r}
Question_51 = exposed$Question_51 %>% replace(is.na(.), "*")
...
```{r}
Question_52 = exposed$Question_52 %>% replace(is.na(.), "*")
...
```{r}
Question_53 = exposed$Question_53 %>% replace(is.na(.), "*")
...
```{r}
Question_54 = exposed$Question_54 %>% replace(is.na(.), "*")
...
```{r}
Question_55 = exposed$Question_55 %>% replace(is.na(.), "*")
...
```{r}
Question_56 = exposed$Question_56 %>% replace(is.na(.), "*")
...
```{r}
Question_57 = exposed$Question_57 %>% replace(is.na(.), "*")
...
```{r}
Question_58 = exposed$Question_58 %>% replace(is.na(.), "*")
...
```{r}
Question_59 = exposed$Question_59 %>% replace(is.na(.), "*")
...
```{r}
Question_60 = exposed$Question_60 %>% replace(is.na(.), "*")
...
```{r}
Question_61 = exposed$Question_61 %>% replace(is.na(.), "*")
...
```{r}
Question_62 = exposed$Question_62 %>% replace(is.na(.), "*")
...
```{r}
Question_63 = exposed$Question_63 %>% replace(is.na(.), "*")
...
```{r}
Question_64 = exposed$Question_64 %>% replace(is.na(.), "*")
...
```{r}
Question_65 = exposed$Question_65 %>% replace(is.na(.), "*")
...
```{r}
Question_66 = exposed$Question_66 %>% replace(is.na(.), "*")
...
```{r}
Question_67 = exposed$Question_67 %>% replace(is.na(.), "*")
...
```{r}
Question_68 = exposed$Question_68 %>% replace(is.na(.), "*")
...
```{r}
Question_69 = exposed$Question_69 %>% replace(is.na(.), "*")
...
```{r}
Question_70 = exposed$Question_70 %>% replace(is.na(.), "*")
...
```{r}
Question_71 = exposed$Question_71 %>% replace(is.na(.), "*")
...
```{r}
Question_72 = exposed$Question_72 %>% replace(is.na(.), "*")
...
```{r}
Question_73 = exposed$Question_73 %>% replace(is.na(.), "*")
...
```{r}
Question_74 = exposed$Question_74 %>% replace(is.na(.), "*")
...
```{r}
Question_75 = exposed$Question_75 %>% replace(is.na(.), "*")
...
```{r}
Question_76 = exposed$Question_76 %>% replace(is.na(.), "*")
...
```{r}
Question_77 = exposed$Question_77 %>% replace(is.na(.), "*")
...
```{r}
Question_78 = exposed$Question_78 %>% replace(is.na(.), "*")
...
```{r}
data_source = exposed$data_source %>% replace(is.na(.), "*")
